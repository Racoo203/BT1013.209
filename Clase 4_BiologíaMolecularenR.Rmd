...
DNAtoRNA <- function(DNAsequence,RNAsequence){
  for(i in (1:length(DNAsequence))){
    if (DNAsequence[i] == "T"){
      RNAsequence[i] <- "U"
    }else{
      RNAsequence[i] <- DNAsequence[i]
    }
  }
  return(RNAsequence)
}

DNAtoRNA(DNAsequence,RNAsequence)
```
...
```{r}
hebraDirec <- c("A","T","G","C","G","C","A","G","A","T","A","A")
hebraInv <- c("","","","","","","","","","","","")

invertirHebra <- function(hebraDirec, hebraInv){
  hebraInv <- rev(hebraDirec)
  return(hebraInv)
}

invertirHebra(hebraDirec,hebraInv)
```
